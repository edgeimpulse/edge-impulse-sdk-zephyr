...
This project differs from `example-standalone-inferencing-zephyr`_ because it uses the `Edge Impulse SDK Zephyr module`_ and the Zephyr library deployment for the model instead of copying the C++ library export.
...
1. Update the ``west.yml`` of your Zephyr repo adding the lines below for the SDK then call ``west update`` to download the SDK into your Zephyr repo.
...
Then update the sample you want to test in ``src/main.cpp``:
